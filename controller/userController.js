        ...
        let user_data= await User.findOne({_id: req.body.userId});
          ...
          await User.updateOne({_id: req.body.userId,pwToken:req.body.pwToken}, update_data)
            .then(async (data) => {
                ...
                {
                  return res.status(200).send({ data:data,status: 200, message: "Password reset successfully" });
                }
                else
                {
                  return res.status(400).send({ data:data,status: 400, message: "Link has been expired!" });
                }
            })
            .catch((err) => {
