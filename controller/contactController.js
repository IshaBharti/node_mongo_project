...
const addcsvcontact = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).send({ status: 200, message: "Inserted successfully" });
  } catch (error) {
  ...
  try {
    ...
    if (contactResult && contactResult._id) {
      ...
      if (result) {
        ...
        return res.status(400).send({ status: 400, message: "Contact not added" });
      }
    }
  } catch (err) {
    return res.status(500).send({
      ...
      const result = await Promise.all(
        contacts.map(async (val) => {
          let contactExist = await Contact.findOne({
            [contactProperty]: contactProperty == "phone" ? val.phone : val.email.toLowerCase(),
            userId: req.user.userId
          });
...
const deleteContact = async (req, res) => {
  ...
  try {
    await Contact.deleteMany({ _id: { $in: contacts } });
...
const singlecontact = async (req, res) => {
  try {
    ...
    return res.status(200).send({ status: 200, message: "Contact Data", data: data });
  } catch (err) {
    return res.status(500).send({
      status: 500,
      message: "Something went wrong, please try again later!",
...
const getContact = async (req, res) => {
  try {
    const data = await Contact.find({ userId: req.user.userId });
...
const skipcontact = async (req, res) => {
  try {
    ...
    return res.status(200).send({ status: 200, message: "succsesfully skipped" });
  } catch (err) {
    return res.status(500).send({
      status: 500,
      message: "Something went wrong, please try again later!",
